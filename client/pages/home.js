...
import { SdzrHero, getHeroVariant } from '../components/sdzr-hero.js';
...
export function HomePage({ socket: _socket }) {
  ...
  // ── 0. TOP MARQUEE ─────────────────────────────────────────────────
  // Lifted from prototype: scrolling drop-info ticker just under the
  // header. Tweaks panel exposes the speed via --sdzr-marquee-speed.
  root.appendChild(buildMarquee('top', 'sdzr-marquee--magenta', [
    'STEMDOMEZ DROP 001', '$2 STL · NO CAD', 'YOUR FACE · YOUR VALVE',
    'FDM · PLA · 0.4mm NOZZLE', 'PRINT IT · RIDE IT', 'SCHRADER FIT ✓',
  ]));

  // ── 1. HERO ───────────────────────────────────────────────────────
  // Tweaks panel writes localStorage['sdz-hero']; if non-default, mount
  // the prototype's alternate hero (cap / card / sheet) in its place.
  const heroVariant = getHeroVariant();
  const altHero = heroVariant !== 'default' ? SdzrHero({ variant: heroVariant }) : null;
  const useDefaultHero = !altHero;
  if (altHero) root.appendChild(altHero);
  // ── 1b. DEFAULT HERO (only when Tweaks chose 'default') ────────────
  const hero = useDefaultHero ? el(
    'section',
    {
  ...
  // Mid marquee — reversed, purple ground.
  root.appendChild(buildMarquee('mid', 'sdzr-marquee--purple', [
  ...
  // Low marquee — ink ground, drop teaser.
  root.appendChild(buildMarquee('low', 'sdzr-marquee--ink', [
...
function buildMarquee(_tag, variantClass, items, opts = {}) {
  // Lifted from prototype's marquee tickers. Repeats items 6× so the
  // CSS animation has enough content to scroll without snapping back.
  const REPEAT = 6;
  const track = el('div', {
    class: 'sdzr-marquee__track' + (opts.reverse ? ' sdzr-marquee__track--rev' : ''),
  });
