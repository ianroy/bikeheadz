...
export function AccountPage({ socket }) {
  ...
  function renderProfile() {
    ...
    profileHeader.append(
      el('div.relative',
        ...
        }, icon('camera', { size: 14, color: '#b4ff45' })),
      ),
      el('div.flex-1',
        el('h1.text-white', { style: { fontWeight: 800, fontSize: '1.3rem' } }, state.displayName),
        el('p', { style: { color: '#808098', fontSize: '0.85rem' } }, state.email),
        el('div.flex.items-center.gap-3.mt-2',
          ...
          el('span', { style: { color: '#606080', fontSize: '0.75rem' } }, '3 designs · 2 orders'),
        ),
      ),
      el('button', {
        class: 'flex items-center gap-1.5 transition-colors',
        style: { color: '#606080', fontSize: '0.8rem' },
      },
        icon('logOut', { size: 16 }),
        'Sign out',
      ),
  ...
  function renderTabs() {
    ...
    for (const tab of tabs) {
      ...
      tabBar.appendChild(el('button', {
        ...
        style: {
          background: active ? '#b4ff45' : 'transparent',
          color: active ? '#000' : '#808098',
          fontWeight: active ? 700 : 500,
          fontSize: '0.85rem',
        },
        onClick: () => { state.activeTab = tab.id; renderContent(); renderTabs(); },
      },
        icon(tab.ico, { size: 16, color: active ? '#000' : '#808098' }),
        tab.label,
      ));
    }
  }
  ...
  function renderDesigns() {
    ...
    for (const d of LOCAL_DESIGNS) {
      wrap.appendChild(el('div', {
        ...
        el('div.flex.items-center.gap-4.p-4',
          ...
          el('div.flex-1',
            el('p', {
              style: { color: '#e0e0f0', fontWeight: 600, fontSize: '0.9rem' },
            }, d.name),
            el('p', { style: { color: '#606080', fontSize: '0.75rem' } }, d.date),
          ...
          el('div.flex.gap-2',
            el('button', {
              ...
              style: { borderColor: '#1e1e35', color: '#808098', fontSize: '0.75rem' },
            },
            ...
            el('button', {
              ...
              style: { background: '#ff6b30', color: '#fff', fontSize: '0.75rem', fontWeight: 600 },
            }, 'Reorder'),
          ),
        ),
      ));
    ...
    wrap.appendChild(el('a', {
      ...
      style: { borderColor: '#252545', color: '#606080', textDecoration: 'none' },
    },
  ...
  function renderOrders() {
    ...
    if (!state.orders.length) {
      wrap.appendChild(el('p', {
        style: { color: '#606080', fontSize: '0.85rem', padding: '1.5rem', textAlign: 'center' },
      }, 'Loading orders…'));
    }
          ...
          el('div.flex-1',
            el('div.flex.items-center.gap-2.mb-1',
              ...
              el('span', {
                class: 'px-2 py-0.5 rounded-full',
                style: {
                  background: `${STATUS_COLORS[order.status] || '#606080'}18`,
                  color: STATUS_COLORS[order.status] || '#b0b0c8',
                  fontSize: '0.68rem',
                  fontWeight: 700,
                  border: `1px solid ${STATUS_COLORS[order.status] || '#606080'}30`,
                },
              }, order.status),
            ),
            el('p', { style: { color: '#606080', fontSize: '0.75rem' } },
              `${order.id} · ${order.date} · Qty: ${order.qty}`,
            ),
          ),
          el('div.text-right',
            el('p', {
              style: { color: '#b4ff45', fontWeight: 700, fontSize: '0.9rem' },
            }, order.price),
            el('button', {
  ...
  function renderSettings() {
    ...
    wrap.appendChild(el('div', {
      ...
      el('div.flex.flex-col.gap-4',
        el('div', { class: 'flex flex-col gap-1.5' },
          el('label', { style: { color: '#808098', fontSize: '0.8rem' } }, 'Display Name'),
          nameInput,
        ),
        el('div', { class: 'flex flex-col gap-1.5' },
          el('label', { style: { color: '#808098', fontSize: '0.8rem' } }, 'Email'),
          emailInput,
        ),
      ),
      el('button', {
    ...
    for (const p of prefs) {
      const row = el('div', {
        ...
        el('span', { style: { color: '#b0b0c8', fontSize: '0.85rem' } }, p.label),
  ...
  function toggleSwitch(initial, onChange) {
    ...
    const track = el('button', {
      ...
      style: {
        ...
        height: '22px',
        border: 'none',
        background: on ? '#b4ff45' : '#252545',
        cursor: 'pointer',
      },
      onClick: () => {
        on = !on;
        track.style.background = on ? '#b4ff45' : '#252545';
