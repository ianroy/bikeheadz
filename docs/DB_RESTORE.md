...
> the StemDomeZ Managed PostgreSQL database to a *fork* (never to prod) and
...
Both are visible in the DO Cloud control panel: **Databases → stemdomez-db → Backups**.
...
```sh
doctl databases fork stemdomez-db \
  --name stemdomez-db-restore-$(date +%Y%m%d) \
...
```sh
doctl databases delete stemdomez-db-restore-<date>
```
